...
impl Fighter {
    ...
    fn eff_attack_speed(&self) -> f32 {
        ...
        for te in &self.effects {
            if let Effect::Freeze { attack_speed_pct, .. } = &te.effect {
                v *= (1.0 + attack_speed_pct / 100.0).max(0.1);
            }
            if let Effect::BeastFrenzy { attack_speed_pct, .. } = &te.effect {
    ...
    fn eff_attack(&self) -> f32 {
        ...
        for te in &self.effects {
            match &te.effect {
                Effect::Berserk { attack_pct, .. } => v *= 1.0 + attack_pct / 100.0,
                Effect::Empower { damage_pct, .. } => v *= 1.0 + damage_pct / 100.0,
                Effect::BeastFrenzy { attack_pct, .. } => v *= 1.0 + attack_pct / 100.0,
                _ => {},
            }
        }
        v.max(0.0)
    ...
    fn eff_defense(&self) -> f32 {
        ...
        for te in &self.effects {
            if let Effect::Fortify { defense_pct, .. } = &te.effect {
    ...
    fn eff_initiative(&self) -> f32 {
        ...
        for te in &self.effects {
            match &te.effect {
                Effect::Haste { initiative_pct, .. } => v *= 1.0 + initiative_pct / 100.0,
                Effect::Paranoia { initiative_pct, .. } => {
    ...
    fn miss_chance(&self) -> f32 {
        let mut m = 0.0;
        for te in &self.effects {
            if let Effect::Blind { miss_pct, .. } = &te.effect {
                m += miss_pct / 100.0;
            }
        }
        m.clamp(0.0, 0.9)
    ...
    fn extra_crit(&self) -> f32 {
        let mut c = 0.0;
        for te in &self.effects {
            if let Effect::Focus { crit_chance_pct, .. } = &te.effect {
    ...
    fn incoming_multiplier(&self) -> f32 {
        let mut v = 1.0;
        for te in &self.effects {
            if let Effect::Vulnerability { damage_pct, .. } = &te.effect {
    ...
    fn can_dodge(&self) -> bool {
        !self
            .effects
            .iter()
            .any(|te| matches!(te.effect, Effect::Immobilize { .. }))
    }

    fn can_act(&self) -> bool {
        !self.effects.iter().any(|te| {
            matches!(te.effect, Effect::Stun { .. } | Effect::MonarchShield { .. })
        })
    }
    ...
    fn lifesteal(&self) -> f32 {
        let mut v = 0.0;
        for te in &self.effects {
            if let Effect::Lifesteal { percentage, .. } = &te.effect {
                v += percentage / 100.0;
            }
        }
        v
...
fn resolve_basic_attack(state: &mut CombatState, attacker: Who, defender: Who) -> Option<(AttackStyle, AttackOutcome)> {
    ...
    if let Some(a) = state.get_mut(attacker) {
        if let Some(pos) = a
            .effects
            .iter()
            .position(|te| matches!(te.effect, Effect::Bleed { .. }))
        {
            if let Effect::Bleed { damage_pct } = a.effects[pos].effect {
    ...
    let thorns: f32 = state
        ...
        .map(|d| {
            d.effects
                .iter()
                .filter_map(|te| {
                    if let Effect::Thorns { damage_reflected_pct, .. } = &te.effect {
...
fn apply_effect(state: &mut CombatState, source: Who, target: Who, effect: &Effect) {
    match effect {
        ...
        Effect::InstantMana { amount } => {
            if let Some(t) = state.get_mut(target) {
                t.restore_mana(*amount as f32);
            }
        },
        Effect::ManaBurn { amount } => {
            if let Some(t) = state.get_mut(target) {
                t.mana = (t.mana - *amount as f32).max(0.0);
            }
        },
        Effect::Manasteal { percentage } => {
            let stolen = state
                .get(target)
                .map(|t| t.mana * percentage / 100.0)
                .unwrap_or(0.0);
            if let Some(t) = state.get_mut(target) {
...
fn effect_duration(effect: &Effect) -> f32 {
    match effect {
        Effect::BeastFrenzy { duration, .. }
        | Effect::Berserk { duration, .. }
        | Effect::Blind { duration, .. }
        | Effect::Burn { duration, .. }
        | Effect::Clearcasting { duration, .. }
        | Effect::Cleave { duration, .. }
        | Effect::Empower { duration, .. }
        | Effect::Focus { duration, .. }
        | Effect::Fortify { duration, .. }
        | Effect::Freeze { duration, .. }
        | Effect::Haste { duration, .. }
        | Effect::Lifesteal { duration, .. }
        | Effect::ManaFlow { duration, .. }
        | Effect::MonarchShield { duration, .. }
        | Effect::Poison { duration, .. }
        | Effect::Paranoia { duration, .. }
        | Effect::Regen { duration, .. }
        | Effect::Silence { duration, .. }
        | Effect::SoulLink { duration, .. }
        | Effect::StatBoost { duration, .. }
        | Effect::Stun { duration, .. }
        | Effect::Taunt { duration, .. }
        | Effect::Thorns { duration, .. }
        | Effect::Immobilize { duration }
        | Effect::Vulnerability { duration, .. } => *duration,
        Effect::Bleed { .. } => 12.0,
        Effect::Curse { timer, .. } => *timer as f32,
        _ => 0.0,
    }
}
...
fn effect_per_second(effect: &Effect) -> (f32, f32) {
    match effect {
        Effect::Burn { damage, .. } | Effect::Poison { damage, .. } => (-(*damage as f32), 0.0),
        Effect::Regen { heal, .. } => (*heal as f32, 0.0),
        Effect::ManaFlow { amount, .. } => (0.0, *amount as f32),
        _ => (0.0, 0.0),
    }
}
...
fn tick_fighter_effects(fighter: &mut Fighter, dt: f32) -> Vec<(FxSide, String, Color)> {
    ...
    for te in fighter.effects.iter_mut() {
        ...
        if let Effect::Curse { damage, .. } = &te.effect {
            if te.remaining <= 0.0 {
...
pub fn combat_tick(
    ...
    for who in [Who::Player, Who::Pet, Who::Enemy] {
        ...
        for (_, text, color) in fx {
            state.fx.push(CombatFx { side, text, color });
        }
    }
        ...
        [(Who::Player, Who::Enemy), (Who::Pet, Who::Enemy), (Who::Enemy, Who::Player)]
    {
        let ready = {
            ...
            f.attack_timer += dt;
            let period = f.attack_period();
            if f.attack_timer >= period {
                f.attack_timer -= period;
                true
            } else {
                false
            }
...
fn try_cast_ability(
    ...
    let enemy_dodged = if has_offensive {
        ...
        state.enemy.can_dodge()
            && rng.random_bool(
                dodge_chance(state.player.eff_initiative(), state.enemy.eff_initiative()) as f64,
            )
    } else {
        false
    };
...
pub fn update_combat_pause_indicator(
    ...
    let paused = state
        .map(|s| s.paused && s.status != CombatStatus::Over)
        .unwrap_or(false);
...
pub fn update_combat_visuals(
    ...
    mut hbar_q: Query<
        &mut Node,
        (
            With<crate::core::ui::playing::HealthBarFill>,
    ...
    mut mbar_q: Query<
        &mut Node,
        (
            With<crate::core::ui::playing::ManaBarFill>,
    ...
    mut pet_hbar_q: Query<
        &mut Node,
        (
            With<crate::core::ui::playing::PetHealthBarFill>,
            Without<crate::core::ui::playing::HealthBarFill>,
            Without<crate::core::ui::playing::ManaBarFill>,
            Without<crate::core::combat::ui::CombatMonsterHealthFill>,
        ),
    >,
    mut monster_hbar_q: Query<
        &mut Node,
        (
            With<crate::core::combat::ui::CombatMonsterHealthFill>,
            Without<crate::core::ui::playing::HealthBarFill>,
            Without<crate::core::ui::playing::ManaBarFill>,
            Without<crate::core::ui::playing::PetHealthBarFill>,
        ),
    >,
    mut overlay_q: Query<(&AbilityCooldownOverlay, &mut Node), (Without<crate::core::ui::playing::HealthBarFill>, Without<crate::core::ui::playing::ManaBarFill>, Without<crate::core::ui::playing::PetHealthBarFill>, Without<crate::core::combat::ui::CombatMonsterHealthFill>)>,
    mut ability_image_q: Query<(&crate::core::combat::ui::AbilityCardImage, &mut ImageNode), (Without<crate::core::ui::playing::HealthBarFill>, Without<crate::core::ui::playing::ManaBarFill>, Without<crate::core::ui::playing::PetHealthBarFill>, Without<crate::core::combat::ui::CombatMonsterHealthFill>)>,
    mut label_q: Query<
        (&mut Text, &crate::core::ui::playing::StatLabel),
    >,
    ...
    if let Ok(mut node) = monster_hbar_q.single_mut() {
    ...
    for (ability, mut image) in &mut ability_image_q {
        let out_of_mana = state
            ...
            .get(ability.slot)
            .map(|slot| {
                slot.key.is_some() && slot.remaining <= 0.0 && state.player.mana < slot.mana_cost as f32
            })
            .unwrap_or(false);
    ...
    if state.status == CombatStatus::Over {
        if let Ok(mut text) = end_btn_text_q.single_mut() {
            let cont = localization.get("general.continue", lang);
            if text.0 != cont {
                text.0 = cont;
            }
        }
    ...
    for f in fx {
        spawn_floating_text(&mut commands, &assets, &f);
    }
}
...
pub fn animate_floating_text(
    ...
    for (entity, mut fct, mut node, mut color) in &mut q {
        ...
        node.top = Val::Percent(fct.start_top - frac * 10.0);
...
/// Removes the combat state when leaving combat.
pub fn cleanup_combat_state(mut commands: Commands) {
